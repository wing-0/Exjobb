...
%	where $\lambda_0$ is vacuum light wavelength, $L/\sin{\theta}$ is the oblique distance of light penetration (approximately), $\mathcal{M}$ is a material parameter and $I_s$ is the acoustic intensity \cite{Saleh2007}. Alternatively, the Bragg condition can be used for the equation
	...
	\begin{multline*}
		\sigma(\theta,\phi) = \frac{\varepsilon_r^2 k^4}{16\pi} \mathfrak{p}^2 S_0^2 L^6 ( \Phi_+(\theta,\phi)^2 + \Phi_-(\theta,\phi)^2 + \\
		+ 2\Phi_+(\theta,\phi) \Phi_-(\theta,\phi) \cos(2\Omega t) )
	\end{multline*}
	where $\Phi_\pm(\theta,\phi)$ is given by
	\begin{multline*}
	\Phi_\pm(\theta,\phi) = \text{sinc} \left( \frac{L}{2\pi} \left( k - k\sin(\theta)\cos(\phi) \pm q\cos(\alpha) \right) \right) \cdot \\
	\cdot \text{sinc} \left( \frac{L}{2\pi} \left( -k\sin(\theta)\sin(\phi) \pm q\sin(\alpha) \right) \right) \cdot \\
