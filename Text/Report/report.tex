...
%	There are two methods of generating radiation force locally. The first method uses amplitude modulated focused ultrasound with its focus on the region of interest \cite{Top2016}. Since amplitude modulated ultrasound exists throughout the beam a force is generated in that entire region. However, the intensity is much higher in the focus so the force is stronger there. The second method instead uses two single-frequency ultrasonic beams which intersect at the region of interest. The frequencies of the two beams differ by a small amount $\Delta \Omega$. Superposition at the intersection will then generate localized amplitude modulation by half the difference frequency $\Delta \Omega/2$. However, the acoustic radiation force is determined by the energy density of the wave. This is proportional to the square of the acoustic field, which changes the frequency of interest by a factor 2. The frequency of the acoustic radiation force is then the difference frequency $\Delta \Omega$ \cite{Fatemi1998}.
	...
	An acoustic wave is usually described using pressure, but it is possible to relate this to a displacement of the particles involved. The relation between pressure and displacement can be written as \cite{Schmerr2016}
	\begin{equation*}
		-\nabla p + \bm{f} = \rho_0 \frac{\partial^2 \bm{u}}{\partial t^2}
	\end{equation*}
	If the body force is set to zero, $\bm{f} = \bm{0}$, and a plane wave is considered, $p(\bm{r}, t) = p_p \cos(\Omega t - qx)$, the equation can be written as
	\begin{equation*}
		\frac{\partial^2 \bm{u}}{\partial t^2} = -\frac{p_p q}{\rho_0} \sin(\Omega t - qx) \bm{\hat{x}}
	\end{equation*}
	This is integrated twice with respect to time to obtain the displacement vector
	\begin{equation*}
		\bm{u}(\bm{r}) = \frac{p_p q}{\rho_0 \Omega^2} \sin(\Omega t - qx) \bm{\hat{x}} + \bm{A}(\bm{r})t + \bm{B}(\bm{r})
	\end{equation*}
	...
	\begin{equation}\label{eq:pressure-displ}
	...
	\begin{equation} \label{eq:strain-displ}
		s_p = qu_p
	\end{equation}
	...
	For ultrasound it is sometimes more common to use acoustic intensity instead of SPL, especially if the propagation is not in air. This is often taken as the time-average of the acoustic power per unit area in the propagation direction \cite{Rossing2014}. It can be related to the strain amplitude as \cite{Saleh2007}
	\begin{equation*}
	...
	\end{align*}
	where $\mathfrak{p}$ was introduced as a scalar photoelastic constant. One thing to note is that since $p_{33} = p_{44} = p_{55} = 0$, shear strain does not have any effect in this simplified model. If the unperturbed permittivity is isotropic, a scalar relation can be written as
	\begin{equation}\label{eq:PE-scalar}
		\Delta \varepsilon_r = -\varepsilon_r^2 \mathfrak{p} \Delta
	\end{equation}
	Here $\Delta \varepsilon_r$ is the isotropic change in relative permittivity and $\Delta$ is the dilatation, which in Cartesian coordinates can be written as $\Delta = s_1 + s_2 + s_3$ \cite{Schmerr2016}. For clarity, it can be good to note that $\Delta \varepsilon_r$ is denoted $\varepsilon_1$ in other sections of this work.
	...
	\end{equation*}
	This can be compared with equation \ref{eq:PE-scalar} to obtain an equivalent photoelasticity \todoint{Check similarity between condensation (relative density change) and dilatation (relative volume change, div of displacements, sum of linear strains)}
	\todoext{All steps are not fully written out here, should be done later}
	\begin{equation}\label{eq:PE-liquid-model}
	...
	\end{equation*}
	The simple model using equations \ref{eq:PE-scalar} and \ref{eq:PE-liquid-model} can be written as (with $\varepsilon_1$ instead of $\Delta \varepsilon_r$)
	\begin{equation*}
		\varepsilon_1 = -\mathfrak{p} \varepsilon_r^2 \Delta = \frac{1}{3}(\varepsilon_r - 1)(\varepsilon_r + 2) s_{xx}
	\end{equation*}
	assuming that the strain can be seen as a plane wave in x near the area of interest. Now, this can be related to pressure using equations \ref{eq:strain-displ} and \ref{eq:pressure-displ} as
	\begin{equation*}
		\varepsilon_1 = \frac{1}{3}(\varepsilon_r - 1)(\varepsilon_r + 2) s_{xx} = \frac{1}{3}(\varepsilon_r - 1)(\varepsilon_r + 2) q u_x = \frac{(\varepsilon_r - 1)(\varepsilon_r + 2) p}{3 \rho_0 v^2}
	\end{equation*}
	...
	\varepsilon_1 (\bm{r}',t) &= -\mathfrak{p} \varepsilon_r^2 s_p \cos(\bm{q} \cdot \bm{r}' - \Omega t)
	\end{align*}
	...
	\begin{figure}[h]
		...
		\caption{\label{fig:simulationflow} Simulation work flow}
	\end{figure}
	...
	An ultrasonic transducer was modeled using the normal acceleration boundary condition. The acceleration was set to "inward acceleration" \todoint{Check this}. The value of the acceleration amplitude was calculated from a displacement amplitude using $a_p = \Omega^2 u_p$ where the displacement amplitude is the same as described in the theory. To reduce the sidelobe level, a taper was added to the transducer by multiplying the acceleration value by a Gaussian taper
	\begin{equation*}
		e^{-(A(-x\sin{\phi} + y\cos{\phi}))^2} = e^{-(A(x\sin{\alpha} - y\cos{\alpha}))^2}
	\end{equation*} \todoint{Derivation for tapering? Rotation of coordinate system and such things then}
	where $A$ affects the width of the taper. An acceptable value for $A$ was found to be $A = 4/d_a$ where $d_a$ is the length of the aperture. \todoint{More info on tapering since this was not used for all simulations (though it's the most general so it should maybe be used)}
	
	\section{Verification of analytical model}
	\todoext{Description of how the simple scattering problem was modeled.}
	To verify the results from section \ref{sec:analytical-radar}
	...
	\begin{align} \label{eq:radar-Aproj}
	...
	\section{Refined interaction region}
	The derivation in \ref{sec:app-derivations-radar} is based on plane waves and a very simple geometry. A problem with the geometry used is that there is no easy way to obtain the parameters $L_x$, $L_y$ and $L_z$ from common parameters such as widths of the acoustic and electromagnetic beams. To address this, a slightly different interaction geometry based on beams is introduced as shown in figure \ref{fig:parallelogram}. Here the beam widths $w_e$ and $w_a$ have been introduced for the electromagnetic and acoustic plane waves. For simplicity, it is assumed that the beam edges are infinitely sharp, i.e. inside the wave behaves as a plane wave and outside it is zero. This way the previous derivation can be used with the exception of the interaction region.
	\begin{figure}[h]
		\centering
		\resizebox{\textwidth}{!}{
	...
	\begin{align*}
		x_0 &= \frac{y}{\tan{\alpha}} \\
		x_1 &= \frac{w_a}{2\sin{\alpha}}
	\end{align*}
	The solution to a general function on the form $e^{i\bm{A} \cdot \bm{r}}$ is now written as
	\begin{align*}
		I &= \int_{V_{sc}} e^{i\bm{A} \cdot \bm{r}'} \mathrm{d}v' \\
		&= \int_{-L_z/2}^{L_z/2} e^{i\bm{A} \cdot \bm{\hat{z}} z'} \mathrm{d}z'
		\int_{-w_e/2}^{w_e/2} \left( e^{i\bm{A} \cdot \bm{\hat{y}} y'}
	...
	\begin{align*}
		I_2 &= \int_{-w_e/2}^{w_e/2} e^{i\bm{A} \cdot \bm{\hat{y}} y'}
		\cdot \frac{1}{i\bm{A} \cdot \bm{\hat{x}}}
		\left( e^{i\bm{A} \cdot \bm{\hat{x}} (x_0+x_1)} - e^{i\bm{A} \cdot \bm{\hat{x}} (x_0-x_1)}\right) \mathrm{d}y' \\
		&=\int_{-w_e/2}^{w_e/2} e^{i\bm{A} \cdot \bm{\hat{y}} y'}
		\cdot e^{i\bm{A} \cdot \bm{\hat{x}} x_0} \frac{2}{\bm{A} \cdot \bm{\hat{x}}} \sin(\bm{A} \cdot \bm{\hat{x}} x_1) \mathrm{d}y' \\
		&=\int_{-w_e/2}^{w_e/2} e^{i\bm{A} \cdot (\bm{\hat{y}} y'+ \bm{\hat{x}} x_0)}
		\cdot 2x_1 \text{sinc}\left( \frac{\bm{A} \cdot \bm{\hat{x}} x_1}{\pi} \right) \mathrm{d}y' \\
		&= \frac{w_a}{\sin{\alpha}} \text{sinc}\left( \frac{\bm{A} \cdot \bm{\hat{x}} w_a}{2\pi \sin{\alpha}} \right) \int_{-w_e/2}^{w_e/2} e^{i\bm{A} \cdot (\bm{\hat{y}} + \bm{\hat{x}}/\tan{\alpha}) y'} \mathrm{d}y' \\
		&= \frac{w_a}{\sin{\alpha}} \text{sinc}\left( \frac{\bm{A} \cdot \bm{\hat{x}} w_a}{2\pi \sin{\alpha}} \right) \cdot I_3
	\end{align*}
	The integral $I_3$ can be solved analogously to the solution from \ref{sec:app-derivations-radar} as
	\begin{equation*}
		I_3 = w_e \text{sinc}\left( \frac{\bm{A} \cdot (\bm{\hat{y}} + \bm{\hat{x}}/\tan{\alpha}) w_e}{2\pi} \right)
	\end{equation*}
	Now the complete solution to $I$ is written as
	\begin{align*}
		I =& \frac{w_a}{\sin{\alpha}} \text{sinc}\left( \frac{\bm{A} \cdot \bm{\hat{x}} w_a}{2\pi \sin{\alpha}} \right) \cdot w_e \text{sinc}\left( \frac{\bm{A} \cdot (\bm{\hat{y}} + \bm{\hat{x}}/\tan{\alpha}) w_e}{2\pi} \right) \\
		&\cdot L_z \text{sinc}\left( \frac{\bm{A} \cdot \bm{\hat{z}} L_z}{2\pi} \right)
	\end{align*}
	If the specific function of interest to the scattering problem is inserted, and spherical coordinates are used as in equation (\ref{eq:radar-Aproj}) the solution is
	\begin{align*}
		\int_{V_{sc}} &e^{i(	k(\bm{\hat{k}} - \bm{\hat{r}}) \pm \bm{q}) \cdot \bm{r}'} \mathrm{d}v' \\
		&= \frac{w_a w_e L_z}{\sin{\alpha}} \text{sinc}\left( \frac{w_a}{2\pi \sin{\alpha}}(k - k\sin{\theta}\cos{\phi} \pm q\cos{\alpha}) \right) \\
		&\cdot \text{sinc}\left( \frac{w_e}{2\pi \tan{\alpha}}
		(k - k\sin{\theta}(\cos{\phi} + \sin{\phi}\tan{\alpha}) \pm q(\cos{\alpha} + \sin{\alpha}\tan{\alpha})) \right) \\
		&\cdot \text{sinc}\left( -\frac{L_z}{2\pi} k\cos{\theta} \right) \\
		&= w_a w_e L_z \Phi_p^{\pm}(\theta, \phi)
	\end{align*}
	where the function $\Phi_p^{\pm}(\theta, \phi)$ was introduced for all angular dependence as for $\Phi^{\pm}(\theta, \phi)$ in \ref{sec:app-derivations-radar}.
	
	\todoint[inline]{Continue derivation, might need to introduce some equation numbering for referencing.}
