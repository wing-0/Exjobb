...
\begin{document}
	...
	\todo[inline]{Now only contains commonly used or in other ways significant symbols. Keep that or expand?}
		Electromagnetics
		\begin{itemize}
		...
		Photoelasticity
		\begin{itemize}
			\item $p_{ij}$ - $ij$ component of the photoelastic tensor
			\item $S_j$ - $j$ component of the strain tensor
			\item $\mathfrak{p}$ - scalar photoelastic constant
			\item $\varepsilon_{r_i}$ - $i$ component of the relative permittivity tensor
			\item $\Delta \varepsilon_{r_i}$ - Perturbation in $i$ component of the relative permittivity tensor
	...
	This mechanism is similar to boundary perturbation in that it is based on scattering from harmonic motion. The difference is that harmonic motion is introduced in a bulk material instead of a resonating discrete target. The mechanism has been utilized in the medical method of vibro-acoustography and harmonic motion imaging \cite{Wang2018}. Both techniques use amplitude modulated ultrasound to produce a time-harmonic acoustic radiation force in a localized region of a sample. \todo{Some more on ARF} This force then generates time-harmonic displacement, or vibration. In the acoustic methods the vibrating region emits acoustic waves which can be detected \cite{Fatemi1998}\cite{Konofagou2003}.
	...
	\todo[inline]{This is a bit of a mess... Need to find good references for everything and standardize notation a bit. The notation needs to be standardized overall, but for some reason the clashes are very apparent here}
	An acoustic wave is usually described using pressure, but it is possible to describe the wave as a periodic displacement of particles. If a plane wave propagating in x is considered, the relation between pressure and displacement amplitudes is then
	\begin{equation*}
		p_{max} = \rho_0 v \Omega d_{max} = q \rho_0 v^2 u_{max}
	\end{equation*}
	where $p_{max}$ is the pressure amplitude, $\rho_0$ is the unperturbed density of the medium, $v$ is the wave velocity, $\Omega$ is the angular frequency, $d_{max}$ is the displacement amplitude and $q$ is the wavenumber. \todo{Citation needed for all of this!}
	...
	\begin{equation*}
		u_x (x,t) = u_0 \cos(\Omega t - qx)
	\end{equation*}
	The only strain resulting from this wave is linear in $x$, and can be written as
	\begin{equation*}
		e_{xx} = \frac{\partial u_x}{\partial x} = qu_0 \sin(\Omega t - qx)
	\end{equation*}
	...
	\end{equation*}
	$p_0$ is a reference pressure which is defined as 20 $\mu$Pa in air. \todo{Citation needed for all this}
	
	For ultrasound it is sometimes more common to use acoustic intensity instead of SPL, especially if the propagation is not in air. This is defined as the time-average of the acoustic power per cross-sectional area. \todo{citation} It can be related to the strain amplitude as \cite{Saleh2007}
	\begin{equation*}
		I_s = \frac{1}{2} \rho_0 v^3 e_0^2
	\end{equation*}
	...
	\begin{align*}
	\Delta \varepsilon_{r_1} &= -\varepsilon_r^2 p_{11} S_0 \cos(\Omega t - qx_1)\\
	\Delta \varepsilon_{r_2} &= \varepsilon_{r_3} = -\varepsilon_r^2 p_{12} S_0 \cos(\Omega t - qx_1)
	\end{align*}
	...
	\end{align*}
	where $\mathfrak{p}$ was introduced as a scalar photoelastic constant. One thing to note is that since $p_{33} = p_{44} = p_{55} = 0$, shear strain does not have any effect in this simplified model. If the unperturbed permittivity is isotropic, a scalar relation can be written as $\Delta \varepsilon_r = -\varepsilon_r^2 \mathfrak{p} S$. Here $\Delta \varepsilon_r$ is the isotropic change in relative permittivity and $S$ is a tensile strain in any direction. For clarity, it can be good to note that $\Delta \varepsilon_r$ is denoted $\varepsilon_1$ in other sections of this work.
	...
	\todo[inline]{Relate back to acousto-optics. Especially in this scalar simplified model since that is similar to what is presented in Saleh.}
	...
	\todo[inline]{Description of how voids were modeled. Relate to delaminations.}
