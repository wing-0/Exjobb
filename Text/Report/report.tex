...
\begin{document}
	...
	\todoext{Now only contains commonly used or in other ways significant symbols. Might change the structure. Symbols probably need to be checked in the report so they are standardized.}
		Electromagnetics
		\begin{itemize}
	...
	This mechanism is based on a discrete target under acoustic resonance. The resonance of the target leads to vibration, which is seen as a time-dependent boundary perturbation \cite{Buerkle2007}. This is seen in the scattered signal as a frequency shift by the vibration frequency \cite{Sarabandi2003}. \todoint{More details on the Radlab stuff. Also describe why this is not used.}
	
	\subsection{Localized Harmonic Motion}
	This mechanism is similar to boundary perturbation in that it is based on scattering from harmonic motion. The difference is that harmonic motion is introduced in a bulk material instead of a resonating discrete target. The mechanism has been utilized in the medical method of vibro-acoustography and harmonic motion imaging \cite{Wang2018}. Both techniques use amplitude modulated ultrasound to produce a time-harmonic acoustic radiation force in a localized region of a sample. \todoint{Some more on ARF} This force then generates time-harmonic displacement, or vibration. In the acoustic methods the vibrating region emits acoustic waves which can be detected \cite{Fatemi1998}\cite{Konofagou2003}.
	...
	\subsection{Acousto-Optics}\todoint{fix this stuff - lots of "old" stuff that isn't very nice to present here. I know much more than when I wrote this...}
	\todoext{Needs to be rewritten since it was written very early in the thesis work, and I don't want to present it in this way.}
	...
	\end{equation*} \todoint{Refine this segment on reflectance and stuff, it's not very coherent}
	...
	\todoint[inline]{See if what's written below works as is or needs editing. It's just copy-pasted from the review doc. Probably need to add even more info than this as well}
	...
	\todoint[inline]{All strains on form $S_j$ have been changed to $s_j$ to standardize notation}
	...
	\end{align*}
	where $\mathfrak{p}$ was introduced as a scalar photoelastic constant. One thing to note is that since $p_{33} = p_{44} = p_{55} = 0$, shear strain does not have any effect in this simplified model. If the unperturbed permittivity is isotropic, a scalar relation can be written as $\Delta \varepsilon_r = -\varepsilon_r^2 \mathfrak{p} \Delta$. Here $\Delta \varepsilon_r$ is the isotropic change in relative permittivity and $\Delta$ is the dilatation, which in Cartesian coordinates can be written as $\Delta = s_x + s_y + s_z$ \cite{Schmerr2016}. For clarity, it can be good to note that $\Delta \varepsilon_r$ is denoted $\varepsilon_1$ in other sections of this work.
	...
	\end{equation*}
	This can be compared with the previous relation $\Delta \varepsilon_r = -\mathfrak{p}\varepsilon_r^2 e$ to obtain an equivalent photoelasticity \todoint{Check similarity between condensation (relative density change) and dilatation (relative volume change, div of displacements, sum of linear strains)}
	\begin{equation*}
	...
	\todoint[inline]{Rewrite this, comes from another section}
	...
	\todoint[inline]{Changed S to e for strains}
	...
	\todoint[inline]{Description of how the simple scattering problem was modeled.}
	
	\section{Effect of voids}
	\todoint[inline]{Description of how voids were modeled. Relate to delaminations.}
