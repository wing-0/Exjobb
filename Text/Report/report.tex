...
\begin{document}
	...
	Nondestructive testing has been ... In industries with high demands for quality and safety, NDT is often a necessity. Aerospace is such an industry and has utilized NDT for a long time. \todo{citation needed} Modern aircraft increasingly use composite materials due to superior properties such as low weight \cite{Katunin2015}. The difference of these materials when compared with traditional metal structures requires changes in the NDT procedures \cite{Riegert2006}. One old technique which is used widely for aerospace composites is ultrasonic testing \cite{Garnier2011}. Another technique which has been developed more recently than ultrasonic testing is microwave/mm-wave imaging, which may hold promises in multiple NDT applications \cite{Kharkovsky2007}.
	...
	\subsubsection{Radio Acoustic Sounding} \todo{Maybe move this part somewhere else?}
	...
	Scattering of electromagnetic waves against a dielectric perturbation is now considered in general. A full derivation can be found in \ref{app:scatterint}, but the main steps are shown here as well. Much of the derivation follows one in \cite{Tatarskii1971}. The starting point is Maxwell's equations for a linear, non-magnetic, source-free, isotropic dielectric. Note that the permittivity is not homogeneous.
	\begin{align*}
	...
	The scattering integral is now solved for a simple geometry and material model to obtain a radar equation. A full derivation can be found in \ref{app:radareq} while some parts have been shortened here for clarity. First, the geometry is defined as shown in figure \ref{fig:radareq-geom-main}.
	\begin{figure}[h]
		\centering
	...
	\end{equation*}
	The solution to this integral is written as (see \ref{app:radareq})
	\begin{equation*}
	...
	\todo[inline]{See if it's possible to slim this subsection down as was done with the previous one. (also for the subsection following this)}
