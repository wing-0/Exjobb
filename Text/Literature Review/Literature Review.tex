...
%	where $\lambda_0$ is vacuum light wavelength, $L/\sin{\theta}$ is the oblique distance of light penetration (approximately), $\mathcal{M}$ is a material parameter and $I_s$ is the acoustic intensity \cite{Saleh2007}. Alternatively, the Bragg condition can be used for the equation
	...
	\subsubsection{Radar range equation}
	...
	\begin{multline*}
		\sigma(\theta,\phi) = \frac{\varepsilon_r^2 k^4}{16\pi} \mathfrak{p}^2 S_0^2 L^6 ( \Phi^+(\theta,\phi)^2 + \Phi^-(\theta,\phi)^2 \\
		+ 2\Phi^+(\theta,\phi) \Phi^-(\theta,\phi) \cos(2\Omega t) )
	\end{multline*}
	where $\Phi^\pm(\theta,\phi)$ is given by
	\begin{multline*}
	\Phi^\pm(\theta,\phi) = \text{sinc} \left( \frac{L}{2\pi} \left( k - k\sin(\theta)\cos(\phi) \pm q\cos(\alpha) \right) \right) \\
	\cdot \text{sinc} \left( \frac{L}{2\pi} \left( -k\sin(\theta)\sin(\phi) \pm q\sin(\alpha) \right) \right) \\
	\cdot \text{sinc} \left( -\frac{L}{2\pi} k\cos(\theta) \right)
	\end{multline*}
	...
	\begin{itemize}
		...
		\item $L$: width of the scattering volume cube
	...
	\begin{itemize}
		...
		\item The scattering volume is approximated by a cube with volume $L^3$.
	\end{itemize}
	...
	\begin{equation*}
	\sigma^\pm(\theta,\phi) = \frac{\varepsilon_r^2 k^4}{16\pi} \mathfrak{p}^2 S_0^2 L^6 \Phi^\pm(\theta,\phi)^2
	\end{equation*}
